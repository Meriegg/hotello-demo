...
const Page = () => {
  ...
  const room = api.rooms.getRoom.useQuery(
    {
    ...
    },
    {
      onSuccess: (data) => {
        ...
        form.setValue("price", data.price / 100);
        form.setValue("accommodates", data.accommodates);
        form.setValue("hasSpecialNeeds", data.hasSpecialNeeds);
        form.setValue("otherAttributes", data.other as any);
      },
    },
  );
  ...
  const addAdditionalAttribute = () => {
    ...
    attributes[uuid()] = additionalAttributeLink.val
      ? {
  ...
  const removeAdditionalAttribute = (attributeKey: string) => {
    const values = form.getValues("otherAttributes");

    delete values[attributeKey];

    form.setValue("otherAttributes", values);
  };
  ...
  const roomActions = () => {
    return (
      <div className="flex flex-wrap items-center gap-4 rounded-lg text-xs font-bold text-red-400 underline">
        ...
        <button
          onClick={() => {
            if (typeof room.data?.isUnavailable !== "boolean") {
              toast({
                variant: "destructive",
                title: "Error",
            ...
            setRoomAvailability.mutate({
              isUnavailable: !room.data.isUnavailable,
              roomId: (params?.roomId as string) ?? "",
            });
          }}
          className="flex items-center gap-2"
        >
          {room.data?.isUnavailable ? "Set as available" : "Set as unavailable"}{" "}
          {room.data?.isUnavailable ? (
  ...
  if (room.isLoading) {
    return <Loader label="Fetching room data" />;
  }
              ...
              {Object.keys(form.watch("otherAttributes")).map((key, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button onClick={() => removeAdditionalAttribute(key)}>
