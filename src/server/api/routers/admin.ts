...
export const adminRouter = createTRPCRouter({
  getAnalyticsData: adminL0Procedure.input(z.object({
    startDate: z.date(),
    ...
    roomBookings.forEach((booking) => {
      const currentCount = roomBookingsCount.get(booking.roomId) ?? 0;
    ...
    const roomsKeyValue = Array.from(roomBookingsCount.keys()).map(
      (
        key: string,
      ) => `${key}:${roomBookingsCount.get(key)}`,
    );

    const bestSellerIds = roomsKeyValue.sort((a, b) =>
      parseInt(b.split(":")[1] ?? "0") - parseInt(a.split(":")[1] ?? "0")
    ).map((key) => key.split(":")[0] ?? "").filter((id) => !!id).slice(0, 2);
    ...
    const bestSellers = bestSellersDbData.map((room) => {
      const numOfBookings = roomsKeyValue.find((key) =>
        key.split(":")[0] === room.id
      )?.split(":")[1] ?? 0;
